package ad.blocker.proxy.handler;

import ad.blocker.proxy.crt.CertPool;
import ad.blocker.proxy.exception.HttpProxyExceptionHandle;
import ad.blocker.proxy.intercept.HttpProxyIntercept;
...
import ad.blocker.proxy.proxy.ProxyConfig;
import ad.blocker.proxy.proxy.ProxyHandleFactory;
import ad.blocker.proxy.server.HttpProxyServer;
import ad.blocker.proxy.server.HttpProxyServerConfig;
import ad.blocker.proxy.server.auth.HttpAuthContext;
import ad.blocker.proxy.server.auth.HttpProxyAuthenticationProvider;
import ad.blocker.proxy.server.auth.model.HttpToken;
import ad.blocker.proxy.util.ProtoUtil;
import ad.blocker.proxy.util.ProtoUtil.RequestProto;
...
public class HttpProxyServerHandler extends ChannelInboundHandlerAdapter
{
    ...
    public void channelRead(final ChannelHandlerContext ctx, final Object msg) throws Exception
    {
        if (msg instanceof HttpRequest)
        {
            ...
            if (getStatus() == 0)
            {
                ...
                // 首次连接处理
                if (getServerConfig().getHttpProxyAcceptHandler() != null && !getServerConfig().getHttpProxyAcceptHandler().onAccept(request, ctx.channel()))
                {
                    setStatus(2);
                    ctx.channel().close();
                    return;
        ...
        else
        {
            ...
            if (getServerConfig().isHandleSsl() && byteBuf.getByte(0) == 22)
            {
                ...
                PrivateKey pKey = getServerConfig().getServerPriKey();
                X509Certificate cert = CertPool.getCert(port, getRequestProto().getHost(), getServerConfig());

                SslContext sslCtx = SslContextBuilder.forServer(pKey, cert).build();
                ctx.pipeline().addFirst("httpCodec", new HttpServerCodec(getServerConfig().getMaxInitialLineLength(), getServerConfig().getMaxHeaderSize(), getServerConfig().getMaxChunkSize()));
    ...
    private void handleProxyData(Channel channel, Object msg, boolean isHttp) throws Exception
    {
        if (getChannelFuture() == null)
        {
            ...
            ProxyHandler proxyHandler = ProxyHandleFactory.build(getInterceptPipeline().getProxyConfig() == null ? proxyConfig : getInterceptPipeline().getProxyConfig());
            ...
            if (isHttp)
            {
                ...
                {
                    // 更新Host请求头
                    if ((pipeRp.getSsl() && pipeRp.getPort() == 443) || (!pipeRp.getSsl() && pipeRp.getPort() == 80))
                    {
