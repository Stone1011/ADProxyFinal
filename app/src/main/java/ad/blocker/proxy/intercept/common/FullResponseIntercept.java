...
public abstract class FullResponseIntercept extends HttpProxyIntercept {
     ...
     */
    private static final int DEFAULT_MAX_CONTENT_LENGTH = (8 << 20);
    ...
    public final void afterResponse(Channel clientChannel, Channel proxyChannel,
        ...
        if (httpResponse instanceof FullHttpResponse) {
            ...
            boolean isFirstMatch = isMatch != null && isMatch;
            // 判断后续的拦截器是否匹配
    ...
    @Deprecated
    /**
     * 剥离到工具类中了：{@link ad.blocker.proxy.util#isHtml(HttpRequest, HttpResponse)}
     */
    protected boolean isHtml(HttpRequest httpRequest, HttpResponse httpResponse) {
        ...
        return httpResponse.status().code() == 200 && accept != null
//                && accept.matches("^.*text/html.*$")
     ...
     * 拦截并处理响应
     */
    public abstract void handleResponse(HttpRequest httpRequest, FullHttpResponse httpResponse,
                               HttpProxyInterceptPipeline pipeline);
}
