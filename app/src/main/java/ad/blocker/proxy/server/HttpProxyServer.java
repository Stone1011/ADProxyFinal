package ad.blocker.proxy.server;
...
import ad.blocker.proxy.crt.CertUtil;
import ad.blocker.proxy.exception.HttpProxyExceptionHandle;
import ad.blocker.proxy.handler.HttpProxyServerHandler;
...
public class HttpProxyServer {
    ...
    private void init() {
        ...
        if (serverConfig.isHandleSsl())
        {
            try
            {
                ...
                {
                    caCert = CertUtil.loadCert(context.getResources().openRawResource(R.raw.ca));
                    caPriKey = CertUtil.loadPriKey(context.getResources().openRawResource(R.raw.ca_private));
                }
                else
                {
    ...
    public HttpProxyServer serverContext(Context con)
    {
        this.context = con;
        return this;
    }
    ...
    public CompletionStage<Void> startAsync(String ip, int port) {
        ...
        channelFuture.addListener(start -> {
            if (start.isSuccess()) {
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N)
                {
                    finalFuture.completeExceptionally(start.cause());
                }
                close();
            }
        });
